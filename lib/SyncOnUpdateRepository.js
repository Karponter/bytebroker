...
const { NoSyncRepository, Datasource } = require('./NoSyncRepository');
const { InMemoryDatasource } = require('./datasource');
...
class SyncOnUpdateRepository extends NoSyncRepository {
   ...
   * @return {Promise}          -- resolves with list of deleted ids
   */
  mdelete(ids) {
    const fluentDefer = Promise.all(this.writeAllowedDatasourceGroup.map((ds) => {
      if (ds.mdelete !== undefined) return ds.mdelete(ids);

      return Promise.all(ids.map(id => ds.delete(id)))
        .then((results) => results.map((result, index) => result ? ids[index] : null))
        .then((results) => results.filter((result) => result !== null));
    }));

    return fluentDefer
      .then((reports) => Array.from(new Set([].concat(...reports))));
  }
